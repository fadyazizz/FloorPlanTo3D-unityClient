...
public class WindowGaps : MonoBehaviour
{
    ...
    public void setBoxColliders(BoxCollider mainBox,BoxCollider left,BoxCollider right)
    {
    ...
    float[] getPoint(Collider other)
    {
        if (other.name == "wall")
        {
            WallMesh scr = other.GetComponentInParent<WallMesh>();
   ...
   GameObject wall() {
    ...
    char getOtherRotation(Collider other)
    {
        GameObject go = other.gameObject;
        if (go.name=="wall")
        {
            return go.GetComponentInParent<WallMesh>().rotation;
        }
        if (go.name == "Door(Clone)")
        {
            return go.GetComponentInParent<Door>().rotation;
        }
        else
        {
            return go.GetComponentInParent<Window>().rotation;
        }
    }
    ...
    void createWall(Collider other)
    {
        ...
        if (whatToFix == "right")
        {
            if (getOtherRotation(other) != rotation)
            {
               
                float x = leftTopMover.center.x;

                float b=GetComponent<MeshFilter>().mesh.bounds.size.x;
            ...
            WallMesh temp= wall().AddComponent<WallMesh>();
            temp.setPoints(otherX2, otherY1, x1, otherY2);
            temp.isFiller = true;
            arr[1] = points;
            count++;
        ...
        if (whatToFix == "left")
        {
            ...
            WallMesh temp = wall().AddComponent<WallMesh>();
            temp.setPoints(x2, otherY1, otherX1, otherY2);
            temp.isFiller = true;
            arr[0] = points;
            count++;
            whatToFix = "";
        ...
        if (whatToFix == "bottom")
        {
            ...
            WallMesh temp = wall().AddComponent<WallMesh>();
            temp.setPoints(otherX1, otherY2, otherX2, y1);
            temp.isFiller = true;
            arr[0] = points;
            count++;
            whatToFix = "";
        ...
        {
            WallMesh temp = wall().AddComponent<WallMesh>();
            temp.setPoints(x1, y2, x2, otherY1);
            temp.isFiller = true;
            arr[1] = points;
            count++;
            whatToFix = "";
...

    {
       ...
       //to solve the problem of perpendicular doors;
        if(other.name== "Door(Clone)"&&gameObject.name== "Door(Clone)")
        {
            if (other.GetComponent<WindowGaps>().rotation != rotation)
            {
                print("in is trigger");
                return;
            }
        ...
        if (!(other.name == "wall" || other.name == "windowWall(Clone)" || other.name == "Door(Clone)"))
        {
            return;
        }
        if (didStart)
